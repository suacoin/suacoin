...
const string strMessageMagic = "Suacoin Signed Message:\n";
...
bool CTxOut::IsDust() const
{
    ...
    // so dust is a txout less than 54 uSUA
                              ...
                              enum GetMinFee_mode mode) const
{
    ...
    if (fAllowFree)
    {
        if (nBlockSize == 1)
        {
            ...
            // (about 4500 SUA if made of 50 SUA inputs)
            if (nBytes < 10000)
                nMinFee = 0;
        }
        else
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SuacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SuacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SuacoinMiner(CWallet *pwallet)
{
    printf("SuacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("suacoin-miner");
    ...
    try { loop {
        ...
        printf("Running SuacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SuacoinMiner terminated\n");
        throw;
    }
}

void GenerateSuacoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SuacoinMiner, pwallet));
}
